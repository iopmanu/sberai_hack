...
class ruDolphApi:
    ...
    def right_text_generation(self, pil_img, r_template='', l_template='', early_stop=64, captions_num=5, seed=None,
                              bs=None, generations_num=32, top_k=None, top_p=None, temperature=None, ppl_txt_w=0.05):
        ...
        ppl_txt, ppl_img, scores = self.self_reranking_by_image(
            texts, pil_img,
            bs=bs, seed=seed, l_special_token='<LT_UNK>', ppl_txt_w=ppl_txt_w
        )
        scores = scores / counts
        result = []
        for idx in scores.argsort()[:captions_num]:
    ...
    def generate_image_text(self, text, template=None, image_prompts=None,
        ...
        right_text = self.generate_text_answers(codebooks[indexes][0], template.format(text=text), top_k=32, top_p=0.8,
                                                special_token='<RT_UNK>')
        return pil_images, result, right_text

    def generate_text_answers(self, image_tokens, left_text, top_k, top_p, temperature=1.0, use_cache=True, template='',
                              allowed_token_ids=None, special_token_left='<LT_UNK>', special_token='<RT_UNK>',
                              texts_num=1):
        '''
